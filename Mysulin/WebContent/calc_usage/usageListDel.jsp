...
<head>
	...
	<%
		...
		Bean_Usage_l lBean = myMgr.getUsageLList(numb1);		//삭제할 자료 가져오기
		
		if(request.getParameter("pass") != null) {
			String inPass = request.getParameter("pass");
			String dbPass = aBean.getUspw();
			if(inPass.equals(dbPass)) {
				myMgr.deleteUsageList(numb1);
				String url = "usageList.jsp?numb=" + numb + "&numb1=0";
...
<body bgcolor="#F0F8FF">
	<div align="center">
		...
		<form name="delFrm" method="post" action="usageListDel.jsp">
				...
				<tr>
					<td align="center">
						<table>
							...
							<tr>
								<td>처방일</td>
								<td><%=lBean.getUsage_date()%></td>
							</tr>
							<tr>
								<td>인슐린코드</td>
								<td><%=lBean.getUsage_code()%></td>
							</tr>
							<tr>
								<td>인슐린이름</td>
								<td><%=lBean.getUsage_name()%></td>
							</tr>
							<tr>
								<td>처방량</td>
								<td><%=lBean.getUsage_total()%></td>
							</tr>
							<tr>
								<td>점검여부</td>
								<td><%=lBean.getUsage_check()%></td>
							</tr>
							<tr>
								<td>패스워드 확인</td>
