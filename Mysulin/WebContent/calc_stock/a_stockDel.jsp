...
<head>
	...
	<%
		...
		Bean_Stock cBean = myMgr.getStock(numb);		//삭제할 자료 가져오기
		
		if(request.getParameter("pass") != null) {
			String inPass = request.getParameter("pass");
			String dbPass = aBean.getUspw();
			if(inPass.equals(dbPass)) {
				myMgr.deleteStock(numb);
				String url = "a_stock.jsp?numb=0";
...
<body bgcolor="#F0F8FF">
	<div align="center">
		...
		<form name="delFrm" method="post" action="a_stockDel.jsp">
				...
				<tr>
					<td align="center">
						<table>
							...
							<tr>
								<td>재고점검일</td>
								<td><%=cBean.getStock_date()%></td>
							</tr>
							<tr>
								<td>인슐린코드</td>
								<td><%=cBean.getStock_code()%></td>
							</tr>
							<tr>
								<td>인슐린이름</td>
								<td><%=cBean.getStock_name()%></td>
							</tr>
							<tr>
								<td>유통기한</td>
								<td><%=cBean.getStock_exp()%></td>
							</tr>
							<tr>
								<td>패스워드 확인</td>
